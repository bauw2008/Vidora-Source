...
class ApiService {
  ...
  /// 获取用户头像
  /// 返回 base64 编码的头像数据，如果没有头像则返回 null
  static Future<ApiResponse<String?>> getAvatar({String? username}) async {
    try {
      final queryParams = username != null ? {'user': username} : null;
      final response = await get<Map<String, dynamic>>(
        '/api/avatar',
      ...
      if (response.success && response.data != null) {
        final avatar = response.data!['avatar'] as String?;
        return ApiResponse.success(avatar, statusCode: response.statusCode);
      } else {
        return ApiResponse.error(response.message ?? '获取头像失败');
      }
    } catch (e) {
      return ApiResponse.error('获取头像异常: ${e.toString()}');
    }
  }

  /// 获取豆瓣周榜数据
  /// [type] 类型: 'movie' 电影周榜, 'tv' 剧集周榜
  /// [limit] 返回数量
  static Future<List<WeeklyHotItem>> getWeeklyHot({
    required String type,
    int limit = 10,
  }) async {
    // 使用缓存：周榜数据缓存7天
    const cacheExpiration = Duration(days: 7);
    final cacheKey = DoubanCacheService().generateWeeklyHotCacheKey(
      type: type,
      limit: limit,
    );

    // 尝试从缓存获取
    final cachedData = await DoubanCacheService().get<List<WeeklyHotItem>>(
      cacheKey,
      (data) => (data as List)
          .map((item) => WeeklyHotItem.fromJson(item as Map<String, dynamic>))
          .toList(),
    );
    ...
    try {
      ...
      if (response.success && response.data != null) {
        final data = response.data!;
        // 服务端返回格式: {success: true, data: {subject_collection_items: []}}
        final responseData = data['data'] as Map<String, dynamic>?;
        final items = responseData?['subject_collection_items'] as List<dynamic>? ?? [];
        final result = items
            .map((item) => WeeklyHotItem.fromJson(item as Map<String, dynamic>))
            .toList();
        
        // 保存到缓存
        await DoubanCacheService().set(cacheKey, result, cacheExpiration);
        
        return result;
      } else {
        print('获取周榜失败: ${response.message}');
        return [];
      }
    } catch (e) {
      print('获取周榜失败: $e');
...
class WeeklyHotItem {
  ...
  factory WeeklyHotItem.fromJson(Map<String, dynamic> json) {
    return WeeklyHotItem(
      id: json['id']?.toString() ?? '',
      title: json['title'] ?? '',
      rating: (json['rating']?['value'] as num?)?.toDouble() ?? 0.0,
      cover: json['cover_url'] ?? json['cover']?['url'] ?? '',
