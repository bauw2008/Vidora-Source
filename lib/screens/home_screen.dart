...
import '../widgets/weekly_hot_section.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// 根据菜单配置构建可用页面列表
  Widget _buildAvailablePages() {
    return PageView.builder(
      controller: _bottomNavPageController,
      itemCount: _getAvailablePageCount(),
      onPageChanged: (index) {
      ...
      itemBuilder: (context, index) {
        ...
        if (_menuSettings.showMovies) {
          if (index == pageIndex) return const MovieScreen();
          pageIndex++;
        }
        if (_menuSettings.showTVShows) {
          if (index == pageIndex) return const TvScreen();
          pageIndex++;
        }
        if (_menuSettings.showAnime) {
          if (index == pageIndex) return const AnimeScreen();
          pageIndex++;
        }
        if (_menuSettings.showVariety) {
          if (index == pageIndex) return const ShowScreen();
          pageIndex++;
        }
        if (_menuSettings.showShortDrama) {
          if (index == pageIndex) return const ShortDramaScreen();
          pageIndex++;
        }
        if (_menuSettings.showLive) {
          if (index == pageIndex) return const LiveScreen();
        }
  ...
  int _getAvailablePageCount() {
    int count = 1; // 首页
    if (_menuSettings.showMovies) count++;
    if (_menuSettings.showTVShows) count++;
    if (_menuSettings.showAnime) count++;
    if (_menuSettings.showVariety) count++;
    if (_menuSettings.showShortDrama) count++;
    if (_menuSettings.showLive) count++;
    return count;
  }
  ...
  Future<void> _refreshHomeData() async {
    try {
      ...
      if (mounted) {
        ...
        // 刷新周榜组件
        await WeeklyHotSection.refresh();
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            ...
            // 海报轮播组件
            PosterCarousel(
              onPosterTap: (videoInfo) {
                _navigateToPlayer(
                  PlayerScreen(
                    title: videoInfo.title,
                    stype: videoInfo.source == 'douban' && videoInfo.rate != null ? 'movie' : null,
                    year: videoInfo.year,
                  ),
                );
              },
  ...
  Widget _buildBottomNavPageView() {
    if (_isLoadingMenuConfig) {
      return const Center(child: CircularProgressIndicator());
    }

    return _buildAvailablePages();
  }
