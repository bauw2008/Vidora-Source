...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  void initVideoData() async {
    ...
    if (widget.source == 'shortdrama' && widget.id != null) {
      updateLoadingMessage('正在获取短剧信息...');
      updateLoadingProgress(0.3);
      updateLoadingEmoji('🎬');
      
      final detail = await ShortDramaService.getDetail(widget.id!);
      
      if (detail != null && detail.episodes.isNotEmpty) {
        
        // 短剧详情获取成功，直接使用 episodes 作为播放列表
        currentDetail = SearchResult(
          ...
          episodesTitles: detail.episodeTitles.isNotEmpty 
              ? detail.episodeTitles 
              : List.generate(detail.totalEpisodes, (i) => '第${i + 1}集'),
          year: detail.year ?? '',
          desc: detail.desc,
          doubanId: 0,
        );
      ...
      // 短剧详情获取失败，继续走搜索流程
      print('短剧详情获取失败，尝试搜索播放源');
      updateLoadingMessage('正在搜索备用播放源...');
  ...
  Future<void> updateVideoUrl(String newUrl, {Duration? startAt}) async {
    print("updateVideoUrl - newUrl: $newUrl, startAt: $startAt");
    
    try {
      String finalUrl = newUrl;
      
      // 检测短剧占位符 URL 格式: shortdrama:id:episodeIndex
      if (newUrl.startsWith('shortdrama:')) {
        final parts = newUrl.split(':');
        if (parts.length == 3) {
          final dramaId = parts[1];
          final episodeIndex = int.tryParse(parts[2]) ?? 0;
          
          print("解析短剧播放地址: id=$dramaId, episode=$episodeIndex");
          
          // 调用解析 API 获取真正的播放 URL
          final playUrl = await ShortDramaService.parsePlayUrl(
            id: dramaId,
          ...
          if (playUrl != null && playUrl.isNotEmpty) {
            finalUrl = playUrl;
            print("短剧解析成功: $finalUrl");
          } else {
            print("短剧解析失败，尝试搜索备用源");
            // 解析失败，尝试搜索
            if (mounted) {
              setState(() {
                _showSwitchLoadingOverlay = true;
                _switchLoadingMessage = '解析失败，正在搜索备用源...';
              });
            }
            
            // 搜索备用源
            final searchResults = await SearchService.searchSync(videoTitle);
            if (searchResults.isNotEmpty) {
              final firstResult = searchResults.first;
              if (firstResult.episodes.isNotEmpty && currentEpisodeIndex < firstResult.episodes.length) {
                finalUrl = firstResult.episodes[currentEpisodeIndex];
                print("使用搜索结果: $finalUrl");
              }
            }
      ...
      print("m3u8ProxyUrl: $m3u8ProxyUrl");

      // 如果代理 URL 不为空，则将 finalUrl encode 后拼接到代理 URL 后面
      ...
      children: [
        if (!_isCasting)
          VideoPlayerWidget(
            ...
            onPipModeChanged: (isPipMode) {
              setState(() {
                _isPipMode = isPipMode;
              });
            },
            onVideoSizeChanged: (width, height) {
              // 更新视频尺寸，触发重新布局（用于竖屏视频）
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: Stack(
                  children: [
                    // PiP 模式：只显示播放器，隐藏所有其他 UI
                    if (_isPipMode)
                      _buildPipModePlayer(theme)
                    else ...[
                      // 主要内容（不包含播放器）
                      if (!_isWebFullscreen)
                        if (_isTablet && !_isPortraitTablet)
  ...
  /// 构建 PiP 模式下的 UI
  /// 重要：Android PiP 窗口显示的是 Activity 内容
  /// 所以必须继续渲染视频，只是隐藏控制栏和其他 UI
  Widget _buildPipModePlayer(ThemeData theme) {
    // PiP 模式下：
    // 1. 继续渲染视频（Android PiP 窗口会裁剪显示）
    // 2. 使用黑色背景
    // 3. 不显示任何控制栏（VideoPlayerWidget 内部会隐藏控制层）
  ...
  /// 根据视频实际尺寸计算播放器高度
  double _calculatePlayerHeight(double width) {
    // 非全屏状态：统一使用 16:9 比例，保证功能区（选集、换源）正常显示
    // 竖屏视频会在播放器内居中显示，左右有黑边
    return width / (16 / 9);
  }
  ...
  Widget _buildPlayerLayer(ThemeData theme) {
    final statusBarHeight = MediaQuery.maybeOf(context)?.padding.top ?? 0;
    final topOffset = statusBarHeight;
    ...
    if (_isWebFullscreen) {
      ...
      if (_isTablet && !_isPortraitTablet) {
        ...
        final playerHeight = _calculatePlayerHeight(leftWidth);
