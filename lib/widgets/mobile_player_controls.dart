...
class _MobilePlayerControlsState extends State<MobilePlayerControls> {
  ...
  void _listenPlayerStreams() {
    ...
    // 监听视频尺寸变化，检测是否为竖屏视频
    _subscriptions.add(widget.player.stream.width.listen((width) {
      if (!mounted) return;
      _videoWidth = width?.toInt();
      _updateVideoOrientation();
    }));
    
    _subscriptions.add(widget.player.stream.height.listen((height) {
      if (!mounted) return;
  ...
  void _updateVideoOrientation() {
    if (_videoWidth != null && _videoHeight != null && _videoWidth! > 0 && _videoHeight! > 0) {
      final isPortrait = _videoHeight! > _videoWidth!;
      if (_isPortraitVideo != isPortrait) {
        setState(() {
          _isPortraitVideo = isPortrait;
        });
      }
    }
  }
  ...
  void _enterFullscreen() {
    _onUserInteraction();
    // 进入全屏（屏幕方向由 VideoPlayerWidget._onEnterFullscreen 处理）
    widget.state.enterFullscreen();
    widget.onFullscreenChange(true);
  }

  void _exitFullscreen() {
    // 退出全屏（屏幕方向由 VideoPlayerWidget._onExitFullscreen 处理）
  ...
  Future<void> _enterPipMode() async {
    debugPrint('MobilePlayerControls: _enterPipMode start');

    // 防止重复点击
    if (widget.isPipMode) {
      debugPrint('Already in PiP mode, ignore');
      return;
    }
    
    // 如果是全屏状态，先退出全屏
    if (_isFullscreen) {
      debugPrint('Exiting fullscreen before PiP');
      widget.state.exitFullscreen();
      widget.onFullscreenChange(false);
      // 恢复所有方向（尊重系统自动旋转设置）
    ...
    // 直接调用 PiP（VideoPlayerWidget 会先调用原生再设置状态）
    debugPrint('Calling onEnterPipMode');
    await widget.onEnterPipMode();
    debugPrint('MobilePlayerControls: _enterPipMode done');
  }
